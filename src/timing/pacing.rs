...
pub fn dwelled_base_secs(cfg: &Config, frequency: i32) -> i64 {
    let mult = 1.0 + cfg.timing.rare_word_dwell * difficulty_factor(frequency);
    (cfg.timing.interval_secs as f64 * mult as f64).round() as i64
}
...
mod tests {
    ...
    fn dwell_off_keeps_uniform_timing() {
        ...
        cfg.timing.rare_word_dwell = 0.0;
    ...
    fn dwell_stretches_rarer_words() {
        let mut cfg = Config::default();
        cfg.timing.interval_secs = 30;
        cfg.timing.rare_word_dwell = 1.0;
        assert_eq!(dwelled_base_secs(&cfg, 1), 30);
